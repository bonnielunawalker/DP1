...
static class MenuController
{
	...
	{
		bool handled = false;
		handled = HandleMenuInput(SETUP_MENU, 1, 1);///<--------I think here is the problem so we will write some unit tests
		if (!handled) {
			HandleMenuInput(MAIN_MENU, 0, 0);
		}
	}
	...
	{
		//this functionality is added so that even if the button is not clicked at the start the game level is set to easy
